...
class ReportsComponent extends React.Component {
  ...
  openEditModal(index, product_id, product_name, editMove_stock, stockType, inout) {
        ...
        return (
          <tr key={report._id}>
            ...
            <td className={!this.state.isPrint ? 'action' : 'actionRemove'}>
              <button className="btn btn-info" style={{marginRight: '10px'}} onClick={() => this.openEditModal(index, product_id, product_name, report.move_stock, report.type, report.inout)}><FaEdit />Moving Stock Edit</button>
            </td>
          </tr>
        );
      }else{
